...
if(scenario == 6)
{
  ...
  #Loads TDA
  load("~/Documents/tesis_mat/master_math_thesis/TDAmapper/interactive_results/MNIST_test_R784/cosine_distance_complete/tsne/TDA/tda.RData")
  #load("~/Documents/tesis_mat/master_math_thesis/TDAmapper/interactive_results/MNIST_test_RP500/cosine_distance_approx_all_angular/tsne/TDA/tda.RData")
  
  location = '~/Documents/tesis_mat/master_math_thesis/TDAmapper/interactive_results/MNIST_test_R784/cosine_distance_complete/tsne/plots/'
}
...
if(scenario == 2)
{
  ...
  location = '~/Documents/tesis_mat/master_math_thesis/TDAmapper/interactive_results/MNIST_test_RP500/cosine_distance_complete_all_angular/tsne/plots/'
}
...
print_table = function(tda_result, dta)
{
  ...
  groupness = round(compute_groupness(tda_result, dta)*100, digits = 1)
  mean_g = round(mean(groupness),1)
...
#colors_cluster = c('deeppink','midnightblue','seagreen1','mediumpurple1','peachpuff4', 'deeppink4','purple', colors)
#adj = tda$result$adjacency
#g  <- graph.adjacency(adj)
#adjusted_adj = adj + identity(tda$result$num_vertices)
...
legend('bottomright', sapply(seq(1,4), function(i){paste('Cluster', i, sep = '')}), 
...
plot(g_original, layout = lay, edge.color = rgb(20,20,20, max = 255))
...
adj = tda$result$adjacency
adj = adj[only_node_ids,only_node_ids]

g  <- graph.adjacency(adj, mode = 'undirected')

centrality = betweenness(g, v = V(g), directed = FALSE, weights = NULL,
            nobigint = TRUE, normalized = FALSE)

centrality = centrality/max(centrality)

V(g)$label = rep(NA, length(V))
V(g)$size = 4
V(g)$color = rgb(1, 0, 0, alpha = centrality, names = NULL, maxColorValue = 1)

plot(g)

sort(-centrality, index.return = TRUE)


png(height = 1000,
    width = 1000,
    file = paste(location,'centrality_',scenario,'.png', sep = '') )


plot(g, layout = lay, edge.color = rgb(20,20,20, max = 255))

legend('bottomright', 'Betweenness', 
       lty=c(1), lwd=c(8), col='red', cex = 2) # gives the legend lines the correct color and width

title(main = 'Layout with Centrality',  cex.main = 3.5)
